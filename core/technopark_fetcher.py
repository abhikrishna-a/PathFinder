...
from config import TECHNOPARK_QUERIES
from core.utils import make_uid, clean_html
...
def _fetch_job_detail(job_id: int, title: str, client: httpx.Client) -> dict:
    ...
    try:
        ...
        decoded = h.unescape(m.group(1))
        data = j.loads(decoded)
        ...
        for page in range(1, 4):
            try:
                ...
                for item in items:
                    ...
                    location = detail.get("address", "") or detail.get("location", "") or location_hint or "Technopark, Trivandrum"
