...
from config import (
    PROFILE, SKILL_WEIGHTS, MATCH_THRESHOLD_APPLY, MATCH_THRESHOLD_TRACK,
    MIN_SALARY, NORTH_INDIA_STATES, MAX_SKILL_GAP_PCT,
...
def _extract_salary(text: str) -> int | None:
    ...
    if any(w in text_lower for w in ["competitive salary", "negotiable", "market rate", "based on experience"]):
        return None
    ...
    for pat in SALARY_PATTERNS:
        ...
        for m in matches:
            ...
            if "lpa" in context or "lakhs" in context:
...
def match_job(job: dict) -> dict:
    ...
    if _is_north_india(location) and "remote" not in location.lower():
        ...
        job["filter_reason"] = "North India on-site only"
    ...
    if not is_target_role:
        ...
        if has_rejected_keyword:
            ...
            job["status"] = "ignored"
            job["relevant_project"] = None
            job["filter_reason"] = f"Not a target role: {job.get('title', '')}"
            job["match_explanation"] = ""
            return job

    salary = _extract_salary(search_text)
    if salary is not None and salary < MIN_SALARY:
        ...
        job["status"] = "ignored"
        job["relevant_project"] = None
        job["filter_reason"] = f"Salary too low (₹{salary:,})"
        job["match_explanation"] = ""
        return job
    ...
    if required_years is not None:
        if required_years > exp_max:
            ...
            job["status"] = "ignored"
            job["relevant_project"] = None
            job["filter_reason"] = f"Experience too high ({required_years}yr required, max {exp_max})"
            job["match_explanation"] = ""
            return job
        if exp_min > 0 and required_years < exp_min:
            ...
            job["status"] = "ignored"
            job["relevant_project"] = None
            job["filter_reason"] = f"Experience too low ({required_years}yr required, min {exp_min})"
            job["match_explanation"] = ""
            return job
    ...
    if total_jd_tech > 0:
        ...
        if match_pct < (100 - MAX_SKILL_GAP_PCT):
            ...
            job["status"] = "ignored"
            job["relevant_project"] = None
            job["filter_reason"] = f"Too many missing skills ({match_pct:.0f}% match, min 60%)"
            job["match_explanation"] = ""
            return job
